# SkincareCustom Theme - Agent Rules

This document contains coding rules and conventions for the SkincareCustom theme that AI agents should follow when making changes.
...
**Note**: Even though Drupal core templates sometimes use `<div{{ attributes }}>`, this pattern is not allowed in SkincareCustom theme components.
...
{{
  include(
    '@skincare_custom/components/icon/icon.twig',
    {
      weight: 'bold',
      icon: icon
    }
...
```twig
{% include '@skincare_custom/components/icon/icon.twig' with {
  weight: 'bold',
  icon: icon
} %}
```
...
**Rule**: After completing any changes to the SkincareCustom theme, always run `npm run format` and `npm run build` to ensure code is properly formatted and the build artifacts are up to date.
...
**Note**: These commands should be run from the SkincareCustom theme directory (`web/themes/custom/skincare_custom/`). Running these commands ensures that:
